...
import { DECO, withReadMore } from './content.js';
...
function virtuesMessage(day) {
  const body = `📜 *قال النبي ﷺ:*
_"ما من أيام العمل الصالح فيها أحب إلى الله من هذه الأيام العشر"_
_(رواه البخاري)_
...
` +
    `• 🗣️ الإكثار من التكبير والتهليل والتحميد
` +
...
` +
  ...
  return withReadMore(DECO.DHU_HIJJAH('🌙 فضل العشر الأوائل من ذي الحجة'), body);
}

function fastingReminder(day) {
  if (day === 9) {
    const body = `🕋 *اليوم هو يوم عرفة*

` +
      `📜 *قال ﷺ:* _"صيام يوم عرفة، أحتسب على الله أن يكفر السنة التي قبله والسنة التي بعده"_
_(رواه مسلم)_
...
` +
    ...
    return withReadMore(DECO.DHU_HIJJAH('🌙 يوم عرفة — أعظم أيام السنة'), body);
  }
  if (day === 10) {
    const body = `🌹 *تقبل الله منا ومنكم صالح الأعمال*
...
` +
    ...
    return withReadMore(DECO.DHU_HIJJAH('🎉 عيد الأضحى المبارك'), body);
  }
  // Days 1-8
  const body = `🌙 *غداً اليوم ${day + 1} من أيام العشر المباركة* 🌟
...
` +
    `🤲 _تقبل الله طاعاتكم_`;
  return withReadMore(DECO.DHU_HIJJAH('🌙 تذكير بالصيام'), body);
}
